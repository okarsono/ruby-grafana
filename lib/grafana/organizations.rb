...
module Grafana
  ...
  module Organizations
    ...
    def organization( organisation_id )
      ...
      raise ArgumentError.new("missing 'organisation_id'") if( organisation_id.empty? )
    ...
    def update_organization( params )
      ...
      return { "status" => 404, "message" => format("Organization '%s' not found", organization) } if( org.nil? || org["status"].to_i != 200 )
    ...
    def organization_users( organization_id )
      ...
      raise ArgumentError.new("missing 'organization_id'") if( organization_id.empty? )
    ...
    def create_organisation( params )
      ...
      return { "status" => 409, "message" => format("Organisation '%s' already exists", name ) } if( org.nil? || org["status"].to_i == 200 )
    ...
    def delete_organisation( organisation_id )
      ...
      if(organisation_id.is_a?(String))
        data = organizations["message"]
        organisation_map = {}
        data.each do |d|
          organisation_map[d["id"]] = d["name"]
        end
